...
export function areNeighbours(a: string, b: string): boolean {
  const country = requireCountry(a);
  return country.neighbours.includes(b);
}
...
export function distancesFrom(start: string): Map<string, number> {
  ...
  for (let head = 0; head < queue.length; head++) {
    ...
    for (const next of requireCountry(current).neighbours) {
 ...
 */
export function shortestRoute(start: string, destination: string): string[] | null {
...
export function shortestMoveCount(start: string, destination: string): number | null {
...
export function connectedComponents(): string[][] {
  ...
  for (const country of COUNTRIES) {
    ...
    const component = [...distancesFrom(country.iso2).keys()];
